...
4. **The `CreationTime` flag (4).** Upstream exposes no process start time, and
   `isWindowsProcessStartTimeAvailable()` gates structured Claude and Codex
   chat on it, so without this hunk win32 silently fell back to the legacy
...
Committed private bytes have no equivalent, and the one memory value the
